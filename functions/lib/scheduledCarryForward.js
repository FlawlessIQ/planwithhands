...
exports.dailyCarryForwardMissedTasks = void 0;
 ...
 */
async function carryForwardForLocation(organizationId, locationId, yesterdayStr, todayStr) {
 ...
 */
exports.dailyCarryForwardMissedTasks = functions
    .runWith({
    memory: "512MB",
    ...
    try {
        ...
        for (const orgDoc of orgsSnapshot.docs) {
            ...
            for (const locDoc of locationsSnapshot.docs) {
                try {
                    const count = await carryForwardForLocation(orgId, locDoc.id, yesterdayStr, todayStr);
        ...
        functions.logger.info(`=== Daily Carry-Forward Complete: ${totalTasksCarriedForward} tasks carried forward ===`);
        return {
            success: true,
