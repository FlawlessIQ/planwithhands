...
async function generateAndSendDailySummary(orgId, date, orgData) {
    ...
    await sendNotificationToAdmins(orgId, titleByLanguage, messageByLanguage, adminUsers);
...
async function getAdminUsers(orgId) {
    try {
        ...
        return usersSnapshot.docs.map(doc => {
            const data = doc.data();
            const email = (data.emailAddress ?? data.email ?? data.userEmail ?? "")
                .toString()
                .trim();
            return {
                userId: doc.id,
 ...
 */
async function sendNotificationToAdmins(orgId, titleByLanguage, messageByLanguage, adminUsers) {
    try {
 ...
 */
function getDailySummaryNotificationCopy(languageCode) {
    if (languageCode === "es") {
        return {
...
function buildDailySummaryNotificationTitle(date, summaryPeriod, languageCode) {
    const copy = getDailySummaryNotificationCopy(languageCode);
    const periodText = summaryPeriod === "business-day" ? ` (${copy.businessDay})` : "";
    return `${copy.summaryTitle} - ${formatDateReadable(date, languageCode)}${periodText}`;
}
function buildNotificationContent(summaryData, date, languageCode = "en") {
    ...
    const performanceEmoji = overallPercentage >= 95 ? "🎉" :
    ...
    if (locationBreakdown && locationBreakdown.length > 0) {
        ...
        for (const loc of locationBreakdown) {
            const emoji = loc.completionPercentage >= 90 ? "✅" :
 ...
 */
function formatDateReadable(date, languageCode = "en") {
    const locale = languageCode === "es" ? "es-US" :
