        ...
        if (tasks.isEmpty) {
          ...
          for (int i = 0; i < tasks.length; i++) {
            ...
            if (title.isEmpty) {
              ...
              print('  ✅ Task ${i + 1}: "$title"${description.isNotEmpty ? ' - $description' : ''}${photoRequired ? ' [PHOTO REQUIRED]' : ''}');
            }
          }
        }
      }
        ...
        if (templateIds.isEmpty) {
          ...
          for (final templateId in templateIds) {
            ...
            if (templateDoc.exists) {
              ...
              if (tasks.isEmpty || tasks.every((task) => (task['title'] ?? task['name'] ?? task['description'] ?? '').isEmpty)) {
                print('    ❌ PROBLEM: This template has empty or missing task titles!');
              }
            } else {
...
class ChecklistDebugWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Checklist Debug Tools'),
      ),
      body: Padding(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Added sample tasks to empty templates')),
                  );
                }
              },
