...
class ForceChecklistCreator {
  ...
  Future<List<DailyChecklist>> createEmergencyChecklists(
    ...
    debugPrint(
      'ForceChecklistCreator: Creating emergency checklists for $organizationId/$locationId on $date',
    );
    ...
    final emergencyTemplates = [
      {
        ...
        'tasks': [
          ...
          {
            'title': 'Verify fire extinguisher locations',
            'description': 'Verify fire extinguisher locations',
            'taskId': 'task2',
            'isCompleted': false,
          },
          {
            'title': 'Confirm first aid kit accessibility',
            'description': 'Confirm first aid kit accessibility',
            'taskId': 'task3',
            'isCompleted': false,
          },
          {
            'title': 'Test emergency communication devices',
        ...
        'tasks': [
          {
            ...
            'description': 'Equipment status check',
            'taskId': 'task1',
            'isCompleted': false,
          },
          {
          ...
          },
          {
            ...
            'description': 'Cleanliness inspection',
            'taskId': 'task3',
            'isCompleted': false,
          },
          {
    ...
    try {
      for (int i = 0; i < emergencyTemplates.length; i++) {
        ...
        final checklistData = {
          ...
          'emergencyCreated':
              true, // Flag to identify emergency-created checklists
        };

        // Save to Firestore
        final docRef = await FirestoreEnforcer.instance
        ...
        debugPrint(
          'ForceChecklistCreator: Created emergency checklist "${template['title']}" with ID ${docRef.id}',
        );
      }
    } catch (e) {
      debugPrint(
        'ForceChecklistCreator: Error creating emergency checklists: $e',
      );
      rethrow;
    }

    debugPrint(
      'ForceChecklistCreator: Successfully created ${createdChecklists.length} emergency checklists',
    );
  ...
  static Future<void> createTestChecklistForShift({
    ...
    try {
      debugPrint(
        '[ForceChecklistCreator] Creating test checklist for shift $shiftId',
      );

      // 1. Create a basic checklist template
      final templateRef = await FirestoreEnforcer.instance
          .collection('organizations')
          .doc(organizationId)
      ...
      debugPrint(
        '[ForceChecklistCreator] Associated template ${templateRef.id} with shift $shiftId',
      );
      ...
      debugPrint(
        '[ForceChecklistCreator] Successfully created daily checklist for today',
      );
    } catch (e, stack) {
