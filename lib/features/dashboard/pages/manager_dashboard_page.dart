...
class _ManagerDashboardPageState extends State<ManagerDashboardPage>
  ...
  final LocationSelectionService _locationSelectionService =
      LocationSelectionService.instance;
  ...
  void initState() {
    debugPrint(
      '[ManagerDashboard] initState() CALLED - Manager Dashboard starting up',
    );
    super.initState();
  ...
  Future<void> _loadCriticalData() async {
    ...
    try {
      ...
      debugPrint(
        '[ManagerDashboard] _loadLiveShifts() completed, now calling _loadYesterdayMissed()',
      );

      // Then load yesterday's missed tasks
      await _loadYesterdayMissed().timeout(const Duration(seconds: 8));
      debugPrint(
        '[ManagerDashboard] _loadYesterdayMissed() completed successfully',
      );
    } catch (e) {
  ...
  Future<void> _loadBackgroundData() async {
    ...
    _loadFrequentMisses30d().timeout(const Duration(seconds: 15)).catchError((
      e,
    ) {
  ...
  void dispose() {
    ...
    _locationSelectionService.listenable.removeListener(
  ...
  Future<void> _onGlobalLocationChanged() async {
    ...
    if (globalId != null && globalId.isNotEmpty) {
      try {
        ...
        logger.w(
          '[ManagerDashboard] _onGlobalLocationChanged reload failed: $e',
        );
      }
    }
  }
      ...
      _shifts =
          shiftsSnap.docs
              .map(
                (d) => {
                  'id': d.id,
                  'name': (d.data()['shiftName'] ?? 'Unnamed Shift').toString(),
                },
              )
              .toList();
      _checklists =
  ...
  Future<void> _loadYesterdayMissed() async {
    ...
    try {
      ...
      debugPrint(
        '[ManagerDashboard] _loadYesterdayMissed: yesterday=${_dateFormat.format(yesterday)}, location=$_selectedLocationId',
      );
        ...
        for (final task in section.tasks) {
          ...
          final group = groupedTasks.putIfAbsent(
            ...
            () => {
              ...
              'count': 0,
              // For "missed yesterday" we only show incomplete tasks, so this stays false.
      ...
      logger.d(
        '[ManagerDashboard] _loadYesterdayMissed: sections=${sections.length} tasks=${sections.fold<int>(0, (s, sec) => s + sec.tasks.length)} grouped=${_yesterdayMissed.length}',
      );
    } catch (e, st) {
  ...
  Future<List<MissedTasksSection>> _loadMissedSectionsForDateDirect(
    ...
    // Resolve shift names (and times if present) once.
    final shiftsSnap =
        await FirestoreEnforcer.instance
            .collection('organizations')
            .doc(widget.organizationId)
  ...
  Future<void> _loadLiveShifts() async {
    ...
    try {
      ...
      for (final shiftDoc in shiftDocs) {
        ...
        if (!_isShiftScheduledToday(shiftData)) {
          logger.i(
            '[ManagerDashboard][DEBUG] Skipping shift $shiftName — not scheduled today',
          );
          continue;
        }
  ...
  Future<void> _loadPoorShifts30d() async {
    ...
    try {
      ...
      try {
        ...
        // Common failure: missing composite index for the range query. We'll fall back to a slower (but safer) path.
        logger.w(
      ...
      if (!usedFastPath) {
        ...
        // Still avoids reading carry-forward tasks and runs in controlled parallel batches.
        ...
        if (preferredLocation != null && preferredLocation.isNotEmpty) {
          ...
          // No selected location; avoid fan-out across all locations on mobile.
          logger.w(
            '[ManagerDashboard][DEBUG] Poor shifts fallback: no selected location; skipping.',
          );
        }
        ...
        Future<void> processChecklist(QueryDocumentSnapshot d) async {
          ...
          // Otherwise, count from subcollection but exclude carry-forward tasks.
          try {
            final tasksSnap =
                await d.reference
                    .collection('tasks')
  ...
  bool _isShiftScheduledToday(Map<String, dynamic> shiftData) {
    final now = DateTime.now();
    ...
    if (!scheduledToday && !hasScheduleMetadata) {
      return true;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (isTablet) {
      ...
      return web_dashboard.ManagerDashboardPage(
        organizationId: widget.organizationId,
      );
    } else {
  ...
  List<ManagerActionIssue> get _attentionIssues {
    ...
    for (final shift in _liveShifts.where(_isShiftAtRisk).take(3)) {
  ...
  List<RecurringIssueSummary> get _recurringFailureItems =>
      _frequentMisses30d.take(4).map((item) {
  ...
  List<RecurringIssueSummary> get _atRiskShiftItems =>
      _poorShifts30d.take(4).map((item) {
        final pct =
    ...
    return GuidedTourHost(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            if (!_metricsEnabled && _showSetupSuggestion) ...[
            ...
            DashboardHeroCard(
              key: _tourHeroKey,
              eyebrow:
                  _selectedLocationName ?? l10n.managerDashboardCurrentLocation,
              headline:
                  isPrimaryLoading
              ...
              child: Column(
                children: [
                  DashboardSectionLabel(
                    ...
                    subtitle: l10n.managerDashboardShiftReadinessSubtitle,
                    helpTopicIds: [
                  ...
                  ShiftReadinessPanel(
                    ...
                    emptyTitle: l10n.managerDashboardNoScheduledShiftsYet,
                    emptySubtitle: l10n.managerDashboardNoScheduledShiftsBody,
                  ),
                ],
              ),
            ),
      ...
      builder:
          (context) => _ProfessionalDialog(
            ...
            height:
                ...
                0.9, // Increased from 0.85 to 0.9 for better mobile experience
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return SizedBox(
          ...
          child: CustomPaint(
            size: Size(w, height),
            painter: _TrendLineChartPainter(values: values),
          ),
        );
      },
    );
...
class _TrendLineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      final y =
          height -
          (normalizedValue * height * 0.85) -
          (height * 0.05); // More space usage, less padding
...
class _LiveShiftStripState extends State<_LiveShiftStrip> {
  ...
  Widget build(BuildContext context) {
    ...
    final inProgress =
        shifts
            .where((s) => s['timeStatus'].toString().contains('remaining'))
            .toList();
              ...
              itemBuilder: (context, index) {
                return Container(
                  ...
                  margin: EdgeInsets.only(
                    right: index < toShow.length - 1 ? 8 : 0,
                  ),
                  child: _SwipeShiftCard(
                    shift: toShow[index],
                    onTap: widget.onOpen,
                  ),
                );
              ...
              itemBuilder: (context, pageIndex) {
                ...
                return Padding(
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder:
                        (context, index) => _SwipeShiftCard(
                          shift: pageShifts[index],
                          onTap: widget.onOpen,
                        ),
                  ),
                );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.comfortaa(
                        ...
                        fontSize:
                            isMobile
                                ? 10
                                : 11, // Slightly smaller text on mobile to fit better
                        color: HandsColors.white,
                      ),
                    ),
                  ),
              ...
              SizedBox(
                height: isMobile ? 2 : 6,
              ), // Tighter spacing to pull time text up
...
class _ProfessionalDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final desiredHeight =
        height ?? (isSmallScreen ? screenHeight * 0.9 : screenHeight * 0.8);
    final maxAvailable =
        screenHeight - insetBottom - (isSmallScreen ? 40.0 : 24.0);
...
class _TaskHistoryDialogState extends State<_TaskHistoryDialog> {
  ...
  int _activeFilterCount() {
    ...
    if (_dateRange != null) {
      if (_dateRange!.start != widget.initialDateRange.start ||
          _dateRange!.end != widget.initialDateRange.end) {
        count++;
      }
    }
    return count;
    ...
    try {
      ...
      for (final d in docs) {
        ...
        for (final t in tasks) {
          ...
          print(
            '[TaskHistory] Debug: Organization ID: ${widget.organizationId}',
          );
          print(
            '[TaskHistory] Debug: Checklist Template ID: ${data['checklistTemplateId'] ?? data['templateId']}',
          );
          print(
          ...
          print(
            '[TaskHistory] Debug: Final photoRequired value: $photoRequired',
          );
          ...
          if (!photoRequired) {
            ...
            print(
              '[TaskHistory] Debug: templateTaskId: $templateTaskId, templateId: $templateId',
            );
            ...
            if (templateTaskId != null &&
              ...
              try {
                ...
                if (templateTaskDoc.exists) {
                  ...
                  print(
                    '[TaskHistory] Debug: Got photoRequired from template: $finalPhotoRequired',
                  );
                } else {
                  print(
                    '[TaskHistory] Debug: Template task not found: $templateTaskId',
                  );
                }
              } catch (e) {
          ...
          if (_selectedCompletion == 'photo_required' && !finalPhotoRequired) {
            print(
              '[TaskHistory] Debug: FILTERED OUT task "$name" - finalPhotoRequired: $finalPhotoRequired',
            );
            continue;
          }
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    icon: Icon(
                      _filtersVisible ? Icons.expand_less : Icons.expand_more,
                      color: HandsColors.white70,
                    ),
                ...
                if (!_loading && _allRows.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(
                        color: HandsColors.sageGreen.withOpacity(0.3),
                      ),
                    ),
                    child: Row(
                      children: [
                      ...
                      if (_displayedRows.length < _allRows.length) ...[
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: HandsColors.cardTertiary,
                            border: Border(
                              top: BorderSide(color: HandsColors.white12),
                            ),
                          ),
                          child: Center(
                            child: LayoutBuilder(
                              builder: (context, constraints) {
                                final remaining =
                                    _allRows.length - _displayedRows.length;
