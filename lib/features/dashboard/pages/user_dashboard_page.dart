      ...
      if (!hasLoadedOnce.value) {
        Future<void> loadDashboardData() async {
          ...
          try {
            ...
            debugPrint(
              "[Dashboard][DEBUG] Found ${foundShifts.length} shifts before filtering",
            );
...
Future<List<ShiftData>> _getAllShiftsForToday(
  ...
  if (userRole == 2) {
    // Admin
    debugPrint(
      "[Dashboard][DEBUG] User is admin, fetching all locations for org $organizationId",
    );
  ...
  debugPrint(
    "[Dashboard][DEBUG] Querying entries for org=$organizationId, userId=$userId, date=$todayString",
  );
  ...
  try {
    ...
    debugPrint(
      "[Dashboard][DEBUG] Found ${shifts.length} published shifts for the user.",
    );
    return shifts;
  } catch (e, stack) {
    debugPrint(
    ...
    for (final locationId in locationIds) {
      ...
      if (publishedScheduleIds.contains(scheduleId)) {
        try {
          ...
          debugPrint(
            "[Dashboard][DEBUG] Found ${entriesSnapshot.docs.length} entries in location $locationId",
          );

          for (final entryDoc in entriesSnapshot.docs) {
            final entryData = entryDoc.data();
            debugPrint(
              "[Dashboard][DEBUG] Entry in $locationId: ${entryDoc.id}, data=$entryData",
            );
            ...
            if (entryData.containsKey('shiftId')) {
              ...
              if (shiftDoc.exists) {
                ...
                allShifts.add(shift);
                debugPrint(
                  "[Dashboard][DEBUG] Added shift: ${shift.shiftName}",
                );
              }
            }
          }
          ...
          debugPrint(
            "[Dashboard][DEBUG] Error querying location $locationId: $e",
          );
        }
      }
    }
...
Future<void> _leaveVolunteerShift(BuildContext context, ShiftData shift) async {
  ...
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Successfully left volunteer shift! Please refresh the page.',
        ),
...
Future<ShiftData?> _checkShiftTemplates(
  ...
  try {
    debugPrint(
      "[Dashboard][DEBUG] Checking shift templates for user $userId, todayDayName=$todayDayName",
    );
    ...
    debugPrint(
      "[Dashboard][DEBUG] assignedShiftsSnapshot.docs.length=${assignedShiftsSnapshot.docs.length}",
    );
    final volunteeredShiftsSnapshot = await volunteeredShiftsQuery.get();
    debugPrint(
      "[Dashboard][DEBUG] volunteeredShiftsSnapshot.docs.length=${volunteeredShiftsSnapshot.docs.length}",
    );
    ...
    for (final doc in assignedShiftsSnapshot.docs) {
      try {
        final data = doc.data();
        debugPrint(
          "[Dashboard][DEBUG] assigned shift doc.id=${doc.id}, data=$data",
        );
        if (!data.containsKey('assignedUserIds')) {
          debugPrint(
            "[Dashboard][DEBUG][ERROR] assigned shift doc.id=${doc.id} missing assignedUserIds field!",
          );
        }
        if (!data.containsKey('days')) {
          debugPrint(
            "[Dashboard][DEBUG][ERROR] assigned shift doc.id=${doc.id} missing days field!",
          );
        }
        ...
        debugPrint(
          "[Dashboard][DEBUG] Failed to parse assigned shift doc ${doc.id}: $e\n$stack",
        );
      }
    }
    ...
    for (final doc in volunteeredShiftsSnapshot.docs) {
      try {
        final data = doc.data();
        debugPrint(
          "[Dashboard][DEBUG] volunteered shift doc.id=${doc.id}, data=$data",
        );
        if (!data.containsKey('volunteers')) {
          debugPrint(
            "[Dashboard][DEBUG][ERROR] volunteered shift doc.id=${doc.id} missing volunteers field!",
          );
        }
        if (!data.containsKey('days')) {
          debugPrint(
            "[Dashboard][DEBUG][ERROR] volunteered shift doc.id=${doc.id} missing days field!",
          );
        }
        final shift = ShiftData.fromJson(data).copyWith(shiftId: doc.id);
        final isToday = shift.repeatsDaily || shift.days.contains(todayDayName);
        ...
        debugPrint(
          "[Dashboard][DEBUG] Failed to parse volunteered shift doc ${doc.id}: $e\n$stack",
        );
      }
    }

    debugPrint(
      "[Dashboard][DEBUG] No shifts found in templates for user $userId",
    );
    return null;
  } catch (e, stack) {
...
Future<ShiftData?> _checkScheduleEntries(
  ...
  try {
    debugPrint(
      "[Dashboard][DEBUG] Checking schedule entries for user $userId on $todayString",
    );
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
    ...
    if (!userDoc.exists) {
      debugPrint(
        "[Dashboard][DEBUG][ERROR] No user document found for userId=${user.uid}",
      );
      return null;
    }
    final userData = userDoc.data()!;
    ...
    if (userRole == 2) {
      ...
      debugPrint(
        "[Dashboard][DEBUG] Admin locationsToCheck: $locationsToCheck",
      );
    } else if (userRole == 1 && userData['locationIds'] != null) {
      locationsToCheck = List<String>.from(userData['locationIds']);
      debugPrint(
        "[Dashboard][DEBUG] Manager locationsToCheck: $locationsToCheck",
      );
    } else if (userData['locationId'] != null) {
      locationsToCheck = [userData['locationId']];
      debugPrint(
        "[Dashboard][DEBUG] General user locationsToCheck: $locationsToCheck",
      );
    }
    debugPrint(
      "[Dashboard][DEBUG] Checking ${locationsToCheck.length} locations for schedule entries",
    );
    for (final locationId in locationsToCheck) {
      final scheduleId = 'schedule_${todayString}_$locationId';
      debugPrint(
        "[Dashboard][DEBUG] Checking scheduleId=$scheduleId for locationId=$locationId",
      );
      try {
        final entriesSnapshot =
            await FirestoreEnforcer.instance
        ...
        for (final entryDoc in entriesSnapshot.docs) {
          try {
            final entryData = entryDoc.data();
            debugPrint(
              "[Dashboard][DEBUG] schedule entry doc.id=${entryDoc.id}, data=$entryData",
            );
            if (!entryData.containsKey('shiftId')) {
              debugPrint(
                "[Dashboard][DEBUG][ERROR] schedule entry doc.id=${entryDoc.id} missing shiftId field!",
              );
            }
            ...
            if (shiftId != null) {
              final shiftDoc =
                  await FirestoreEnforcer.instance
                      ...
                      .get();
              debugPrint(
                "[Dashboard][DEBUG] shiftDoc.exists=${shiftDoc.exists} for shiftId=$shiftId",
              );
              if (shiftDoc.exists) {
                final shift = ShiftData.fromJson(
                  shiftDoc.data()!,
                ...
                debugPrint(
                  "[Dashboard][DEBUG][ERROR] shiftDoc not found for shiftId=$shiftId",
                );
              }
            }
          } catch (e, stack) {
            debugPrint(
              "[Dashboard][DEBUG] Error processing schedule entry ${entryDoc.id}: $e\n$stack",
            );
          }
        }
      } catch (e, stack) {
        debugPrint(
          "[Dashboard][DEBUG] Error checking location $locationId: $e\n$stack",
        );
      }
    }
    debugPrint(
      "[Dashboard][DEBUG] No shifts found in schedule entries for user $userId",
    );
    return null;
  } catch (e, stack) {
    debugPrint(
      "[Dashboard][DEBUG] Error checking schedule entries: $e\n$stack",
    );
    return null;
  }
}
...
class _ChecklistCard extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final completedTasksCount =
        checklist.tasks.where((task) => task.isCompleted).length;
          ...
          ListTile(
            ...
            trailing: Row(
              ...
              children: [
                ...
                if (task.notCompletedReason != null &&
                    task.notCompletedReason!.isNotEmpty)
                  IconButton(
                    icon: Icon(
                      Icons.warning,
                      size: 20,
      ...
      final shouldContinue = await showDialog<bool>(
        ...
        builder:
            (context) => AlertDialog(
              ...
              content: const Text(
                'This task requires a photo to be completed. Would you like to add a photo now?',
              ),
              actions: [
                TextButton(
                  onPressed: () => Navigator.pop(context, false),
  ...
  Future<void> _toggleTask(BuildContext context, bool isCompleted) async {
    try {
      ...
      if (user == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("You must be logged in to complete tasks"),
          ),
        );
        return;
      }
                ...
                children: [
                  ElevatedButton.icon(
                    onPressed:
                        isUploading.value
                            ? null
                            : () => _replacePhoto(context, isUploading),
                  ...
                  OutlinedButton.icon(
                    onPressed:
                        isUploading.value
                            ? null
                            : () => _removePhoto(context, isUploading),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed:
              isSaving.value
                  ? null
                  : () => _saveNotes(context, notesController.text, isSaving),
          child: const Text('Save'),
        ),
        if (task.notes != null && task.notes!.isNotEmpty)
          TextButton(
            onPressed:
                isSaving.value ? null : () => _deleteNotes(context, isSaving),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed:
              isSaving.value
                  ? null
                  : () => _saveReason(context, reasonController.text, isSaving),
          child: const Text('Save'),
        ),
        if (task.notCompletedReason != null &&
            task.notCompletedReason!.isNotEmpty)
          TextButton(
            onPressed:
                isSaving.value ? null : () => _deleteReason(context, isSaving),
            child: const Text('Delete', style: TextStyle(color: Colors.red)),
          ),
      ],
    );
          ...
          children:
              snapshot.data!.docs.map((doc) {
                ...
                return ListTile(
                  ...
                  onTap:
                      () => onLocationSelected({'id': doc.id, ...locationData}),
                );
              }).toList(),
        );
      },
    ...
    return FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(
            child: Text("No shifts found for this location."),
          );
        }
            ...
            snapshot.data!.docs
                .map((doc) {
                  try {
                    final shift = ShiftData.fromJson(
                      doc.data(),
                    ).copyWith(shiftId: doc.id);
                    // Additional check for today using activeDays as well as days
                    if (shift.repeatsDaily ||
                        shift.days.contains(todayDayName) ||
                        shift.activeDays.contains(todayWeekday)) {
                      return shift;
                    }
                  } catch (e, stack) {
                    debugPrint(
                ...
                .whereType<ShiftData>()
                .toList(); // Filter out all the nulls
