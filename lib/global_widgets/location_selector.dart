...
class AllowedLocationsResolver {
  ...
  Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>> streamAllowedLocations({
    required int userRole,
    required String orgId,
    required List<String>? assignedLocationIds,
    required List<DocumentReference<Map<String, dynamic>>>? assignedLocationRefs,
  }) {
    ...
    if (userRole == 2) {
      ...
      return locationsCol.where('isActive', isEqualTo: true).snapshots().map((s) => s.docs);
    }
    ...
    if (uniqueIds.length <= 10) {
      return locationsCol.where(FieldPath.documentId, whereIn: uniqueIds).snapshots().map((s) => s.docs);
    }

    // Chunk into multiple streams and combine manually: listen to each snapshot stream and merge
    final controllers = <Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>>>[];
    for (var i = 0; i < uniqueIds.length; i += 10) {
      final slice = uniqueIds.sublist(i, (i + 10).clamp(0, uniqueIds.length));
      controllers.add(locationsCol.where(FieldPath.documentId, whereIn: slice).snapshots().map((s) => s.docs));
    }
    ...
    final latest = List<List<QueryDocumentSnapshot<Map<String, dynamic>>>>.filled(controllers.length, []);
    final subs = <StreamSubscription<List<QueryDocumentSnapshot<Map<String, dynamic>>>>>[];
