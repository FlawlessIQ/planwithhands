...
final unreadNotificationsCountProvider = StreamProvider<int>((ref) {
  ...
  if (userState.userData == null) {
    debugPrint('[unreadNotificationsCountProvider] userData not loaded yet for user ${user.uid}. Waiting...');
    // Return a stream that will be replaced when userData loads
    return Stream.value(0);
  }
  ...
  return FirestoreEnforcer.instance
      ...
      .handleError((error) {
        debugPrint('[unreadNotificationsCountProvider] Stream error: $error');
        // Return 0 on error to prevent crashes, but log the issue
        return 0;
      });
});
